...
│   │   ├── FindLeadsDialog.tsx   # Lead finding with background job support
...
│   │   └── web-search/           # DuckDuckGo search for lead finding
...
- [x] API route endpoints:
  ...
  - `/api/search` - DuckDuckGo web search for lead finding
  - `/api/domains/[id]/check` - DNS/HTTP check for domain variants
...
- [x] Four lead finding strategies (`src/lib/lead-strategies.ts`):
...
- Zustand store (`src/stores/useJobsStore.ts`) manages job state
- Jobs can be minimized to floating cards while user continues working
- `MinimizedJobs` component renders floating progress cards at bottom-right
- Sidebar shows "Active Jobs" section for running/completed jobs
- Jobs persist until user dismisses them (success toast pattern)
- `DashboardClient` wrapper adds `MinimizedJobs` to dashboard layout

### Lead Finding Strategies
- Strategies defined in `src/lib/lead-strategies.ts`
- Domain variants generated by `src/lib/domain-variants.ts`
- Web search via DuckDuckGo (Google blocked by CAPTCHA)
- Blocked domains list filters news/blogs/directories from results
- Real-time lead creation: leads added during scraping, not after
...
10. **Zustand stores** - use for client-side state that needs to persist across component unmounts (e.g., background jobs)
11. **DuckDuckGo search** - Google search is blocked by CAPTCHA; use DuckDuckGo HTML search (`html.duckduckgo.com`)
12. **Puppeteer** - use regular `puppeteer` package, not `puppeteer-extra` (causes build errors)
